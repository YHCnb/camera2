...
class GlRenderView @JvmOverloads constructor(context: Context?, attrs: AttributeSet? = null) :
    ...
    fun setCameraMode(mode: CameraMode){
        queueEvent{ glRender.setCameraMode(mode) }
    }

    fun setSaturation(saturation:Int){
        queueEvent { glRender.setSaturation(saturation) }
    }

    fun setContrast(contrast:Int){
        queueEvent { glRender.setContrast(contrast) }
    }

    fun setExposure(exposure:Int){
        queueEvent { glRender.setExposure(exposure) }
    }

    fun setBrightness(brightness:Int){
        queueEvent { glRender.setBrightness(brightness) }
    }
